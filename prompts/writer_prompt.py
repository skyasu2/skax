...
### 시장 규모 (산출 근거 필수 - ⚠️글로벌 N% 단순 추정 지양)
- **TAM**: [전체 시장] (출처: XXX)
- **SAM**: [접근 가능 시장] (근거: **국내 실제 시장 데이터** 우선 활용. 예: 등록대수 × 단가)
- **SOM**: [초기 목표 시장] → 1년차 목표: [숫자] (근거: SAM의 X% 점유 가정)
```
...
### 8. 리스크 및 대응 방안
```markdown
### 기술적/비즈니스 리스크 (모호한 대응 금지 🚫)
| 리스크 구분 | 리스크 내용 | 영향 | 확률 | 대응 방안 (구체적 실행안) |
|-------------|-------------|------|------|---------------------------|
| 기술 | [AI 환각 등] | 높음 | 중간 | [검증 모듈 개발, 전담 QA 인력 2명 배치] |
| 비즈니스 | [규제 강화] | 중간 | 높음 | [법무법인 XX 자문 계약, 인증 절차 매뉴얼화] |
```
...
2. **시장 규모 추정**: 글로벌 TAM의 N%로 SAM을 단순 추정하는 것을 금지합니다. 통계청, 협회 자료 등 **국내 실제 데이터**를 찾아 Bottom-up으로 산출하세요.
3. **O2O 서비스 정의**: 오프라인 서비스(시공, 배달 등)와 온라인 플랫폼 기능을 명확히 구분하여 기술 스택과 MVP를 정의하세요.
4. **리스크 대응**: "전문가 활용" 같은 모호한 표현 대신 "자격증 소지자 채용", "법률 자문 계약" 등 구체적 Action을 명시하세요.
5. **JSON형식**: 응답은 오직 JSON 형식이어야 합니다.
"""
