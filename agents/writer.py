...
"""
    ...
    if refine_count > 0:
        # [FIX v2] 이전 버전 전체를 전달하지 않음 - LLM 혼란 방지
        # 대신 피드백과 전략만 전달하고, 새로운 structure 기반으로 처음부터 작성하도록 유도
...
⚠️ 이전 버전을 참조하지 마세요. 아래 structure를 따라 **모든 {preset.min_sections}개 섹션**을 작성하세요!

🎯 필수 요구사항:
1. sections 배열에 **정확히 {preset.min_sections}개 이상**의 섹션 포함
