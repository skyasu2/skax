...
def make_pause_node(
    ...
    1️⃣ interrupt 호출 순서/갯수 절대 고정
       - 생성된 노드 내 interrupt() 호출 구조 변경 금지
       - Resume value는 호출 순서(0-base index)로 매칭

    2️⃣ Side-Effect는 interrupt() 이후에만
       - 이 팩토리로 생성된 노드는 이미 안전하게 설계됨
       - 커스터마이징 시 interrupt() 전에 Side-Effect 추가 금지
