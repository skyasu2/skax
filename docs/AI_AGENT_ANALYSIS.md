...
### C. 능동적 웹 검색 (Active Search) - *New!*
- **기능**: `Market Agent`가 초기 데이터 부족 시 **스스로 검색 쿼리를 생성**하여 실시간 웹 데이터를 수집합니다.
- **제어**: 무한 루프 방지를 위해 실행당 최대 2회 검색으로 제한되며, 비용과 속도를 최적화합니다.

### D. 고화질 분할 작성 (High-Fidelity Chunk Writing) - *New!*
- **문제**: 긴 호흡의 글 작성 시 후반부 섹션이 누락되거나 내용이 부실해지는(Context Loss) 현상 발생.
